...
class ComplexArray{
  ...
  */
  ...
  get arg (){
    ...
    let angles = new Array(this.length);
    for (let i = 0; i < this.length; i++){
      angles[i] = this.getArgElement(i);
    }
    return angles;
  }
  push (x, y){
...
class AnimationObject{
  ...
  itvl_lngth: Number of animation frames between
              the each j interval
  x_cvs: x canvas coordinates for plotting a point
  y_cvs: y canvas coordinates for plotting a point 
...
Ani.animate = function(){
  ...
  */
  ...
  Ani.ctx.beginPath();
  Ani.ctx.strokeStyle = 'rgba(0, 0, 155, 1)';
  ...
  Ani.updateCircles(Ani.FT_Output.length/2);
  Ani.ctx.stroke();
  Ani.ctx.closePath();

  Ani.ctx.beginPath();
  //Ani.ctx.strokeStyle = 'rgb(255,165,0)';
  Ani.ctx.strokeStyle = 'rgba(0, 0, 0, 1)';
  ...
  for (let i = j; i >= 0; i--){
    ...
    Ani.prevPoint.x = Ani.Input_2D.real[i];
    Ani.prevPoint.y = Ani.Input_2D.imag[i];
  }
  Ani.ctx.stroke();
  Ani.ctx.closePath();
  Ani.j += Ani.increment();
